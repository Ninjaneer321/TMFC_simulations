function parallel_calculate_FIR_PPIs(stat_path,exp_folder,N,N_ROIs)
...
for subji = 1:N
    for j = 1:N_ROIs
        ...
        matlabbatch{1}.spm.stats.ppi.name = ['FIR_TaskA_' num2str(j,'%.3d')];
        ...
        matlabbatch{1}.spm.stats.ppi.name = ['FIR_TaskB_' num2str(j,'%.3d')];
        matlabbatch{1}.spm.stats.ppi.disp = 0;
        ...
        matlabbatch{1}.spm.stats.ppi.name = ['FIR_TaskA_vs_TaskB_' num2str(j,'%.3d')];
        matlabbatch{1}.spm.stats.ppi.disp = 0;
...
for subji = 1:N
    for j = 1:N_ROIs
        TaskA = load([stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep 'PPI_FIR_TaskA_' num2str(j,'%03.f') '.mat']);
        TaskB = load([stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep 'PPI_FIR_TaskB_' num2str(j,'%03.f') '.mat']);
        TaskA_vs_TaskB = load([stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep 'PPI_FIR_TaskA_vs_TaskB_' num2str(j,'%03.f') '.mat']);
    ...
    save([stat_path filesep exp_folder filesep 'GLMs' filesep 'Sub_' num2str(subji,'%.3d') filesep  'PPI_FIR_ALL.mat'],...
